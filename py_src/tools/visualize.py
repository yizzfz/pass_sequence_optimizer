...
def main():
    ...
    for i, program_id in enumerate(test_list):
        ...
        if((scaled_O3 > 0 and scaled_O3 < 100)or 1):
            ...
            all_eva_mean.append(sum(evaluated_result) /
                                float(len(evaluated_result)))
    ...
    rects2 = ax.bar(ind + width, all_eva_best, width,
                    label='Best Prediction', alpha=opacity)
    rects3 = ax.bar(ind + width * 2, all_eva_mean, width,
                    label='Average Prediction', alpha=opacity)
