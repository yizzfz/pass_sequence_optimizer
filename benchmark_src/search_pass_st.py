...
def save_list(child_list, filename):
...
# given a list, randomly select n element from it
...
def record_list(prefix, time, size, child_list, cnt):
    subdir = 'data_' + prefix + str(cnt)
    os.system('mkdir -p ' + subdir)
    save_list(child_list, subdir + '/passList.txt')
...
def ga(parent_list, current_dir, time_baseline, size_baseline, cnt, succ, fail, O3_time):
    ...
    for i in range(0, CHILD_MAX):
        cnt += 1
        child_list = CPS.generate_list(parent_list)     # recompile LLVM with a random list
        res = create_child(child_list, current_dir, time_baseline)
        if res[0] == 0:
            child_time = res[1]
            child_size = res[2]
            ...
            if time_improve > 0.5 and child_time < best_time:  # choose this child if improvment is greater than ?%
                ...
                size = child_size
                improved = 1
                record_list('A', child_time, child_size, child_list, cnt)
                succ += 1
            else:
                record_list('B', child_time, child_size, child_list, cnt)
        else:
            fail += 1
...
def create_child(child_list, current_dir, time_baseline):
...
def read_list(target_dir):
    ...
    pass_list = load_list(target_dir + '/' + optimal + '/passList.txt')
    return pass_list
...
def t_prepare(current_dir):
    ...
    record_list('O3_', O3time, O3size, CPS.O3_list, 0)
...
def t_GA(current_dir, all_list, O3_list, testcodes):
    ...
    for i in range(0, GA_MAX):
        res = ga(current_list, current_dir,
