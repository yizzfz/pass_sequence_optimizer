...
data_types = ['half', 'float', 'double', 'i1', 'i8', 'i16', 'i32',
              'i64', 'i1*', 'i8*', 'i16*', 'i32*', 'i64*', 'vector', 'other']
performance_metrics = [
...
def main():
    ...
    for i in range(0, rng):
        ...
        tup = (bench_list[i], bench_path[i], O0data[i],
               O3data[i], opt[i], similar[i])
        all_data.append(tup)
        if opt[i] == None:
...
def get_passlist(file):
    ...
    try:
        ...
        args1 = [pass_dict[x]
                 for x in args if x.startswith('-') and pass_dict.has_key(x)]
        return args1
    except IOError:
        print ("Error: File does not appear to exist :" + file)
