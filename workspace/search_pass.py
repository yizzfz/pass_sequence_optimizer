...
def record_list(prefix, time, size):
    global cnt
    cnt += 1
      ...
      if res[0] == 0:
        ...
        if time_improve > 0.5:      #choose this child if improvment is greater than ?%
          if child_time < best_time:
            ...
            size = child_size
            improved = 1
            record_list('A', child_time, child_size)
        else:
          record_list('B', child_time, child_size)
...
def thread_func(current_dir, all_list, O3_list, testcodes):  
    cnt=0
    os.chdir(current_dir)
    O3_list_str = list_to_string(O3_list)
    #print 'init list: ', O3_list_str 
    ...
    if err==0:
      ...
      print 'cannot compile with O3'
    
    print 'O3 time = ', O3time, ', bin size = ', O3size
    record_list('O3_', O3time, O3size)
